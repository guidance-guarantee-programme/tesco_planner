...
RSpec.describe 'POST /sms_cancellations', type: :request do
  ...
  scenario 'successfully cancelling an existing appointment' do
    ...
    and_the_underlying_slot_is_free
  ...
  def given_a_pending_appointment_exists
    @appointment = create(:appointment, :with_slot, phone: '07715930455')
  end
  ...
  def and_the_underlying_slot_is_free
    expect(Slot.available.find_by(start_at: @appointment.slot.start_at)).to be
  end
